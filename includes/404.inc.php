<section>
<div id="logo-region" style="display: none;">
  <div class="row">
    <div class="small-8 large-3 large-offset-0 small-offset-2 columns logo">
	<a href="./" class="internal"><img src="./img/logo_small.png"></a>
    </div>
    <div class="large-9 small-12 columns main-search-box" style="margin-bottom: 0">
	<input id="searchBox" type="text" placeholder="Search for block, transaction or address in " style="font-size: 1.2em;" />
    </div>
  </div>
</div>
</section>
</header>
</div>
</div>
<div id="main-region"><div><div class="row">
   <div class="small-9 large-8 small-centered columns" style="text-align: center;margin-top: 4.5em">
       <img src="./img/logo_big.png">
   </div>
</div>
<div class="row">
	<div class="small-12 large-8 small-centered columns" style="text-align: center;margin-top: 0.9em">
		<div class="panel radius"><h5>404 Error</h5><p>The page that you are looking, does not exist!</p></div>
	</div>
</div>
</div></div>
<div id="push"></div>
</div>
