...
while ($stmt->fetch()) {
	...
	if($type == "credit") {
		$received_txs++;
		$received_zet = bcadd($received_zet, $value, 8);
	} elseif($type == 'debit') {
		$sent_txs++;
		$sent_zet = bcadd($sent_zet, $value, 8);
	}
...
*/
	...
	$txrows .= '
				<tr>
					<td class="blocksHash">
						<a href="./index.php?engine=blockexplorer&blockid=' . $blocknum . '" class="internal transactionLink">' . $blocknum . '</a></td>
					<td class="transactionHash">
						<a href="./index.php?engine=blockexplorer&tx=' . $tx . '" class="internal transactionLink">' . $tx . '</a>
					</td>
					...
					<td class="transactedAmount" style="text-align: right;">' . number_format($value, 8, '.', ',') . ' ZET</td>
				</tr>';
	}
}
        ...
        <table width="100%">
			...
			<tr>
				<td class="tableRowLabel">FirstSeen</td><td class="tableRowValue">
					<a href="./index.php?engine=blockexplorer&hash=<?php echo $firstseen; ?>" class="internal transactionLink"><?php echo gmdate("M j Y g:i:s A", $time); ?> - Block n. <?php echo $height; ?></a>
				</td>
			</tr>
            <tr><td class="tableRowLabel">Total Received</td><td class="tableRowValue"><?php echo number_format($received_zet, 8, '.', ','); ?> ZET</td></tr>
            <tr><td class="tableRowLabel">Total Sent</td><td class="tableRowValue"><?php echo number_format($sent_zet, 8, '.', ','); ?> ZET</td></tr>
			 <tr><td class="tableRowLabel">Current Balance</td><td class="tableRowValue"><b><?php echo number_format($received_zet - $sent_zet, 8, '.', ','); ?> ZET</b></td></tr>
        </tbody></table>
    </div>
